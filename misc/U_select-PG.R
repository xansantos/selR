...
# Once you have decided (based on AICc) which is the best candidate model to model your data, you simply has to conduct the boostrap on the data stacked  haul by haul
...
#The function to conduct the boostrap is called B_select_win(), and the following arguments needs to be included in the function
...
# lp=c(10,65) = same values you used in the estimation of the average curve and parameters (using F_select)

#n.min=1 = same values you used in the estimation of the average curve and parameters (using F_select)

#fix_par=list("sp",.5) = same values you used in the estimation of the average curve and parameters (using F_select)

#B=1000 : number of pseudosamples in the boostrap scheme 1000 is frequently used

#ncores=2 : The number of cores you want to use
...
B_select_win(X=X,Start=start,modelo=2,ref_length=NULL,lp=c(10,100),n.min=1,fix_par=list("sp",.5),B=1000,ncores=4,exportfuns=exportfuns)
